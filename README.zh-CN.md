...
KeepFacts 当前文档所述的浏览器工作流已经功能完备，v0.3.2 将这套工作流转入稳定维护，v0.3.3 继续收紧无障碍和审阅流程。项目仍是 pre-1.0 的 `0.x` 版本：这不代表 KeepFacts 已成为 v1，也不承诺 API、文件格式或识别结果永远兼容。仅最新标签版本按 best-effort 原则维护，不提供 LTS 分支或响应时限 SLA；详见[隐私与边界](#隐私与边界)和[安全政策](SECURITY.md)。

后续补丁版本仅处理安全、缺陷、兼容性、无障碍、文档和发布设施问题。新增产品能力或事实识别器必须进入范围明确的版本，并提供对应证据。可直接运行的示例仍是首要操作，并在首屏预告结果中的三处差异；不含补丁版本号的 1200 × 630 社交卡、canonical URL、Open Graph、X/Twitter 卡片和 favicon 让分享链接长期保持一致；源码、隐私与反馈入口保持可见，移动端交互控件高度不小于 44 CSS 像素。

v0.3.3 进一步压缩首屏，让核对区更快进入视野；增加键盘可见的“跳到核对区”入口，并在人工选择结论后才展开可选备注和期望修复。重复标签和不可用操作已清理，移动端保留率标签不再挤成多行，品牌首页入口也纳入 44 CSS 像素触控标准；未保存的用户输入在清空或载入示例前会得到与人工记录相同的保护，较长的英文首屏也不会把 390 px 下的核对区完全推出视野。既有核对逻辑没有改变。

示例首屏改为非对称编辑排版，并只保留一处高对比度“事实校对单”作为视觉焦点。进入操作区后，原文与改写稿回到完全对称的浅色校样纸，编号标题更紧凑，结果记录也合并为成组面板；移动端将三处示例差异改为纵向记录，不再压成迷你仪表盘。整套界面继续沿用纸张、墨色、深绿和浅黄视觉语言。
...
你可以主动导出或导入本地 `.keepfacts.json` 会话，以转移当前草稿、上次核对输入和人工记录。该文件有严格版本与字段约束，但属于未加密明文。这里的 schema v1 仅表示会话数据格式版本，不表示 KeepFacts 产品已进入 v1；详见 [会话格式 v1](docs/session-format-v1.md)。

v0.3.2 没有扩张 v0.3.1 的事实识别规则或公开评测语料；它统一了编辑器与会话文件的输入边界，加速了长必保清单，并加强了失败后的安全恢复。v0.3.3 仅调整界面与无障碍行为，同样不表示识别范围或准确率已经提高。KeepFacts 适合检查 AI 转换前后已识别硬事实是否保真；即使“已提取事实保留率”为 100%，也不代表所有说法真实、所有事实都已识别或全文语义完全保留。
