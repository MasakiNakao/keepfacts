...
[![KeepFacts v0.2 原文与改写事实核对及人工审阅界面](docs/keepfacts-demo.jpg)](https://masakinakao.github.io/keepfacts/)
...
4. 点击“开始核对”生成一份固定结果；把异常项标记为“确认需处理”“改写合理”或“已忽略”，再复制或下载 Markdown 报告。修改文本后请重新核对。
...
核对时，KeepFacts 使用精确十进制字符串规范化金额、数量和百分比，避免浮点数舍入；重复或重排的数值则通过附近语境进行一对一关联。点击核对后，完整比较会在浏览器本地 Web Worker 中运行，避免大文本分析阻塞输入操作。人工结论不会改写自动保留率，也不会写入浏览器存储；它们只存在于当前页面，并在复制或下载时写入报告。
