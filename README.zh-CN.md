...
KeepFacts 当前文档所述的浏览器工作流已经功能完备，v0.3.2 将这套工作流转入稳定维护。项目仍是 pre-1.0 的 `0.x` 版本：这不代表 KeepFacts 已成为 v1，也不承诺 API、文件格式或识别结果永远兼容。仅最新标签版本按 best-effort 原则维护，不提供 LTS 分支或响应时限 SLA；详见[隐私与边界](#隐私与边界)和[安全政策](SECURITY.md)。
...
v0.3.2 没有扩张 v0.3.1 的事实识别规则或公开评测语料；本次统一了编辑器与会话文件的输入边界，加速了长必保清单，并加强了失败后的安全恢复，因此不表示识别范围或准确率已经提高。它适合检查 AI 转换前后已识别硬事实是否保真；即使“已提取事实保留率”为 100%，也不代表所有说法真实、所有事实都已识别或全文语义完全保留。
